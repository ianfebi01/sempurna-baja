...
const { data: productData, pending: _productPending } = await useAsyncData( `product-${slug.value}`,
  () => $fetch<ApiSuccess<{ data: Product[] }>>( "/api/products", {
    params: {
      slug: slug.value,
    },
  } ),
...
const { data: uniqueCategoryData, pending: _uniqueCategoryPending } = await useFetch<ApiSuccess<ProductResponse>>( "/api/products/unique-category", {
